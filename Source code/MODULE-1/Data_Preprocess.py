...
path = os.path.dirname('E:/Effective Heart Disease Identification/MODULE-1/')
...
df.to_csv(os.path.join(path, 'preprocess_data/combined_dataset.csv'), index=False)

import matplotlib.pyplot as plt
from matplotlib import rcParams
from matplotlib.cm import rainbow

dataset=pd.read_csv('preprocess_data/combined_dataset.csv')
print(dataset)
dataset.describe()
dataset.hist()

rcParams['figure.figsize'] = 8,6
plt.bar(dataset['num'].unique(), dataset['num'].value_counts(), color = ['red', 'green','blue','yellow','brown'])
